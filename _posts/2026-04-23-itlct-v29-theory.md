...
> **摘要**：本文提出了基于非平衡统计物理的意识理论——**临界协同理论 (Critical Synergy Theory, CST)**。我们将意识量化为**因果协同 (Causal Synergy, $\Phi_{CS}$)**，即系统在最大熵干预下，其整体未来状态不可约于部分未来状态的信息量。通过**接触过程 (Contact Process)** 模型，我们提供了数值证据表明：$\Phi_{CS}$ 在系统从吸收相到活跃相的**相变临界点**达到最大值。该理论将意识从纯粹的信息论概念转化为一种**临界可观测量 (Critical Observable)**，并给出了严格的证伪条件。
...
为了解决这些问题，**ITLCT v29.0** 引入了**因果干预**和**非平衡动力学**。我们引入了一个核心概念：**因果协同 (Causal Synergy)**，定义为：
$$\Phi_{CS} = EI - R_{causal}$$
这里我们减去冗余是为了**分离出不可约的因果影响 (Irreducible Causal Influence)**。有效信息 ($EI$) 捕捉的是系统整体的因果约束，但它包含了可以通过观察局部状态预测的部分（即冗余）。减去冗余后，剩余的 $\Phi_{CS}$ 专门衡量那些必须由整体系统共同决定的信息，这正是“整体大于部分之和”的本质。
...
*图 1: 机制图。有效信息 (EI) 衡量总因果力，减去冗余后，剩余部分即为协同。*
...
### 2.2 关于偏信息分解 (PID) 的说明
在定义 $R_{causal}$ 时，我们采用了 $I_{min}$ 作为冗余的保守估计。虽然 PID 的具体度量（如 $I_{min}, I_{ccs}, I_{dep}$ 等）在学术界仍有讨论，但我们的核心假设是：$\Phi_{CS}$ 作为一个表征系统整合能力的量，其**临界峰值特征**在不同 PID 定义下应具有鲁棒性。未来的工作将使用更严格的度量对此进行交叉验证。

### 2.3 系统划分
为了计算 $\Phi_{CS}$，我们采用**空间二分法 (Spatial Bipartition)**，将一维环形晶格分为两个连续的半圆部分。这种划分方式旨在捕捉系统在空间上不同区域之间的长程因果依赖。

---

## 3. 物理模型：非平衡临界动力学的最小模型

我们采用**接触过程 (Contact Process)** 作为研究非平衡相变动力学的最小模型，而非直接模拟大脑。该模型虽然简单，但抓住了复杂系统在临界态的关键特征。
...
我们的数值模拟结果表明：**$\Phi_{CS}$ 在临界点 $\lambda_c$ 处达到最大值。**
...
2.  **非冗余性 (Non-Redundancy)**：在活跃相（$\lambda \gg \lambda_c$），系统处于高密度活跃状态。虽然此时系统的动力学趋于确定（EI 可能较高），但由于大部分节点状态趋于一致，**因果冗余 (Redundancy)** 会急剧上升。
    由于大部分信息已包含在局部状态中，**协同信息 ($\Phi_{CS}$)** 反而会下降。
...
这意味着系统整合因果信息的能力随规模呈超线性增长（Super-linear scaling）。

**理论联系 (Theoretical Connection)**：
值得注意的是，接触过程属于**定向渗流 (Directed Percolation, DP)** 普适类。我们观测到的标度指数 $\gamma$ 很可能与 DP 的临界指数存在深层联系。
目前的数值 $\gamma \approx 1.3$ 是基于有限尺寸系统的初步结果。未来的工作将致力于在更大的系统规模下验证该指数是否收敛于由 DP 理论导出的特定值。
