...
为了解决这些问题，**ITLCT v29.0** 引入了**因果干预**和**非平衡动力学**，定义了一个新的可观测量：**因果协同 (Causal Synergy)**。

我们引入：
$$\Phi_{CS} = EI - R_{causal}$$

这里我们减去冗余是为了**分离出不可约的因果影响**。有效信息 (EI) 衡量系统整体的因果约束，但它包含了可以通过观察局部状态预测的部分（即冗余）。减去冗余后，剩余的 $\Phi_{CS}$ 专门衡量那些必须由整体系统共同决定的信息，这正是“整体大于部分之和”的本质。
...
### 2.2 关于偏信息分解 (PID) 的说明
在定义 $R_{causal}$ 时，我们采用了 $I_{min}$ 作为冗余的保守估计。虽然 PID 的具体度量（如 $I_{min}, I_{ccs}$ 等）在学术界仍有讨论，但我们的核心假设是：$\Phi_{CS}$ 作为一个表征系统整合能力的量，其**临界峰值特征**在不同 PID 定义下应具有鲁棒性。未来的工作将使用更严格的度量（如 BROJA 或 $I_{dep}$）对此进行交叉验证。

### 2.3 系统划分
为了计算 $\Phi_{CS}$，我们需要定义系统的划分方式。在本研究中，我们采用**空间二分法 (Spatial Bipartition)**，将一维环形晶格分为两个连续的半圆部分（$A$ 和 $B$）。这种划分方式旨在捕捉系统在空间上不同区域之间的长程因果依赖，是衡量系统全局整合能力的关键。

---

## 3. 物理模型：非平衡临界动力学的最小模型

我们采用**接触过程 (Contact Process)** 作为研究非平衡相变动力学的最小模型。
