...
\end{equation}
where $\alpha_{\text{coupling}} \approx 0.1$-0.3 is the boundary-bulk coupling coefficient (related to but distinct from T416's $\alpha_{\text{coupling}}^{\text{(T416)}} \approx 10^{-2}$; the relationship is $\alpha_{\text{coupling}}^{\text{(T417)}} \approx \alpha_{\text{coupling}}^{\text{(T416)}} \times \Phi_{\text{max}}$), $\Phi_{\text{max}}$ is the system's maximum $\Phi$ capacity (bits), and the correction term is negligible when $\Gamma_{\text{dec}} \gg \Gamma_{\text{sys}}$.
